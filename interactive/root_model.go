...
func (m *RootModel) renderRobotRun() string {
	...
	if m.Logs != nil && m.Logs.Len() > 0 {
		...
		if len(entries) > 0 {
			...
			b.WriteString("\n")
			b.WriteString(m.Styles.Subtle.Render("> " + lastLine))
		}
	}
...
func (m *RootModel) renderRunComplete() string {
	...
	b.WriteString(formatRow("Robot", m.Styles.Title.Render(robotName)))
	...
	// Timing
	if m.RobotState.BuildTime > 0 {
		b.WriteString(formatRow("Build", m.Styles.Subtle.Render(formatDurationShort(m.RobotState.BuildTime))))
	}
	...
	// Log viewer section
	b.WriteString(m.Styles.Subtle.Render(strings.Repeat("─", m.contentWidth())))
	b.WriteString("\n")
	...
	if len(logLines) == 0 {
		b.WriteString(m.Styles.Subtle.Render("  No log output available"))
		b.WriteString("\n")
	} else {
		...
		for i := start; i < end; i++ {
			...
			if strings.Contains(line, "[PASS]") || strings.Contains(line, "PASS") {
				...
				lineStyle = m.Styles.Info
			} else {
				lineStyle = m.Styles.ListItem
			}
		...
		if len(logLines) > visibleLines {
			...
			b.WriteString(m.Styles.Subtle.Render(scrollPos + scrollHint))
		}
	}
