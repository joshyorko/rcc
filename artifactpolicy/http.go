// Package artifactpolicy contains acyclic policy shared by artifact provider
// implementations and settings references.
...
// NormalizeHTTPURL validates and canonicalizes an Environment Artifact HTTP
// provider URL. Errors intentionally omit the raw URL so embedded userinfo is
// never echoed to callers.
func NormalizeHTTPURL(raw string) (string, error) {
	parsed, err := url.Parse(raw)
	if err != nil || parsed.Scheme == "" || parsed.Host == "" || parsed.User != nil || parsed.RawQuery != "" || parsed.Fragment != "" {
		return "", fmt.Errorf("invalid artifact provider URL")
	}
