...
func newEnvironmentExportCommand(dependencies environmentCommandDependencies) *cobra.Command {
	...
	command := &cobra.Command{Use: "export", Short: "Export a verified environment artifact as an offline archive.", Args: cobra.NoArgs, SilenceUsage: true,
		RunE: func(command *cobra.Command, _ []string) error {
			if artifact == "" || providerURL == "" || output == "" { return fmt.Errorf("--artifact, --provider, and --output are required") }
			digest, err := environmentartifact.ParseDigest(artifact); if err != nil { return err }
			if dependencies.newProvider == nil { return fmt.Errorf("environment provider dependency is unavailable") }
			provider, err := dependencies.newProvider(providerURL); if err != nil { return err }
			manifest, err := environmentlifecycle.ExportArchive(command.Context(), environmentlifecycle.ExportArchiveRequest{ArtifactDigest: digest, Provider: provider, OutputPath: output}); if err != nil { return err }
