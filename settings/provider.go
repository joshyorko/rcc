...
func (it ProviderProfile) Validate() (ProviderProfile, error) {
	...
	normalized, err := normalizeProviderURL(it.URL)
	if err != nil {
...
// normalizeProviderURL mirrors the artifactprovider HTTP boundary. Settings
// cannot import artifactprovider because its Provider implementation depends on
// environmentartifact, whose legacy catalog path imports settings.
