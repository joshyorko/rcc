...
func Repair(ctx context.Context, digest environmentartifact.Digest) (RepairReport, error) {
	...
	err := withContentTransaction(ctx, localContentRoot(), func(ctx context.Context) error {
		return withArtifactTransaction(ctx, digest, func(ctx context.Context) error {
			...
			if in.Lease.Active > 0 {
				report = RepairReport{Inspection: in, Reconciled: in.Lease}
				return fmt.Errorf("%w: refuse repair while materialization is leased", ErrActiveLease)
			}
			if in.Corrupt {
...
func RepairFromProvider(ctx context.Context, digest environmentartifact.Digest, provider artifactprovider.Provider) (RepairReport, error) {
	...
	err := withContentTransaction(ctx, localContentRoot(), func(ctx context.Context) error {
		return withArtifactTransaction(ctx, digest, func(ctx context.Context) error {
			reconciled, reconcileErr := reconcileLocked(ctx, digest)
			if reconcileErr != nil {
				return reconcileErr
			}
			if reconciled.Active > 0 {
				report.Inspection = Inspection{Digest: digest, Lease: reconciled}
				return fmt.Errorf("%w: refuse provider repair while materialization is leased", ErrActiveLease)
			}
