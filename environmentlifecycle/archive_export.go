...
func ExportArchive(ctx context.Context, request ExportArchiveRequest) (environmentartifact.Manifest, error) {
	...
	if err != nil {
		return environmentartifact.Manifest{}, fmt.Errorf("resolve artifact manifest: %w", err)
	}
	manifest, err := environmentartifact.DecodeManifest(manifestBytes)
	if err != nil {
		return environmentartifact.Manifest{}, err
	}
	if manifest.ArtifactDigest != request.ArtifactDigest {
		return environmentartifact.Manifest{}, fmt.Errorf("resolved manifest identity does not match requested artifact")
	}
	...
	for _, item := range []struct {
		...
		if err != nil {
			return environmentartifact.Manifest{}, fmt.Errorf("fetch archive member %s: %w", item.name, err)
		}
	...
	for _, object := range index.Entries {
		...
		if err != nil {
			return environmentartifact.Manifest{}, fmt.Errorf("fetch archive object %s: %w", object.LegacyObjectID, err)
		}
...
func writeArchiveAtomically(output string, entries map[string][]byte) error {
	...
	if err := environmentartifact.WriteArchive(temporary, entries); err != nil {
		_ = temporary.Close()
		return err
	}
