...
func cleanupAnywork(t *testing.T) {
	t.Helper()
	t.Cleanup(func() { _ = anywork.Sync() })
}

func TestBacklogProcessesWorkAndSyncReportsSuccess(t *testing.T) {
	cleanupAnywork(t)
	...
	if got := completed.Load(); got != 1 {
		t.Fatalf("processed work count = %d, want 1", got)
	}
}
...
func (it *recordingCloser) Close() error { it.closed = true; return nil }
