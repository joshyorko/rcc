...
func NewPlatformIndex(specification Digest, artifacts []PlatformArtifact) (PlatformIndex, []byte, error) {
	...
	if err := index.Validate(); err != nil { return PlatformIndex{}, nil, err }
...
func DecodePlatformIndex(content []byte) (PlatformIndex, error) {
	...
	if err := strictDecodeCanonical(content, &index); err != nil { return PlatformIndex{}, err }
	if err := index.Validate(); err != nil { return PlatformIndex{}, err }
	return index, nil
}

func (index PlatformIndex) Validate() error {
	if index.MediaType != PlatformIndexMediaType || index.SchemaVersion != 1 || index.Specification.Hex() == "" { return fmt.Errorf("invalid platform index") }
	seen := make(map[string]bool, len(index.Artifacts))
	for _, item := range index.Artifacts {
		if err := item.Platform.Validate(); err != nil { return err }
		if item.Artifact.Hex() == "" || seen[item.Platform.RCCPlatform] { return fmt.Errorf("duplicate or empty platform artifact") }
		seen[item.Platform.RCCPlatform] = true
	}
	if len(index.Artifacts) == 0 { return fmt.Errorf("platform index has no artifacts") }
	return nil
}

func (index PlatformIndex) Select(platform Platform) (Digest, error) {
	if err := platform.Validate(); err != nil { return Digest{}, err }
	for _, item := range index.Artifacts { if item.Platform == platform { return item.Artifact, nil } }
