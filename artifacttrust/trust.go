...
func NewSBOM(artifact string, components []Component) (SBOM, []byte, error) {
	...
	for _, c := range copyComponents {
		if containsCredential(c.Source) {
			return SBOM{}, nil, fmt.Errorf("SBOM source contains credentials")
		}
	}
...
func CanonicalProvenance(p Provenance) ([]byte, error) {
	...
	for _, s := range append([]string{p.SourceRepository}, p.DependencySources...) {
...
func containsCredential(value string) bool {
	...
	if strings.Contains(lower, "password=") || strings.Contains(lower, "passwd=") || strings.Contains(lower, "secret=") || strings.Contains(lower, "token=") || strings.Contains(lower, "apikey=") || strings.Contains(lower, "api_key=") {
		return true
	}
	u, err := url.Parse(value)
	return err == nil && u.User != nil && (u.User.Username() != "" || func() bool { _, ok := u.User.Password(); return ok }())
}
func VerifyAttestationBinding(data []byte, mediaType, artifact string) error {
	...
	if v.MediaType != mediaType || v.ArtifactDigest != artifact {
...
func SignAt(artifact, keyID string, privateKey ed25519.PrivateKey, notBefore, notAfter time.Time) (Signature, error) {
	s, err := Sign(artifact, keyID, privateKey)
	if err == nil {
		s.NotBefore = FreshTimestamp(notBefore)
		s.NotAfter = FreshTimestamp(notAfter)
	}
	return s, err
}
...
func (p Policy) Evaluate(local bool, artifact, platform, builder string, signatures []Signature, revocations []Revocation, keys map[string]ed25519.PublicKey) error {
	...
	for _, s := range signatures {
		...
		if key := keys[s.KeyID]; key != nil {
			if err := VerifySignature(s, key, artifact); err == nil && signatureActive(s, time.Now().UTC()) {
				return nil
			}
		}
	}
...
func (p Policy) Verify(q VerifyRequest) VerificationReceipt {
	...
	if q.Provenance != nil && q.Provenance.ArtifactDigest != q.ArtifactDigest {
		r.Code = CodeBinding
		r.Diagnostic = "provenance artifact digest mismatch"
		return r
	}
	if q.SBOM != nil && (q.SBOM.MediaType != SBOMMediaType || q.SBOM.ArtifactDigest != q.ArtifactDigest) {
		r.Code = CodeBinding
		r.Diagnostic = "SBOM artifact digest mismatch"
		return r
	}
	if q.Provenance != nil {
		if !containsOrEmpty(p.AcceptedRCCVersions, q.Provenance.RCCVersion) || !containsOrEmpty(p.AcceptedSources, q.Provenance.SourceRepository) {
			r.Code = CodePolicy
			r.Diagnostic = "provenance disallowed by policy"
			return r
		}
		if p.MaxArtifactAge > 0 {
			if t, e := time.Parse(time.RFC3339, q.Provenance.CreatedAt); e == nil && q.At.Sub(t) > p.MaxArtifactAge {
				r.Code = CodePolicy
				r.Diagnostic = "artifact exceeds maximum age"
				return r
			}
		}
	}
	if err := p.Evaluate(p.Mode == PermissiveLocal, q.ArtifactDigest, q.Platform, q.Builder, q.Signatures, q.Revocations, q.Keys); err == nil {
		r.Valid = true
		r.Code = CodeValid
		return r
	} else {
	...
	if len(q.Signatures) == 0 {
		...
		r.Code = CodeInvalid
		for _, s := range q.Signatures {
			if q.Keys[s.KeyID] == nil {
				r.Code = CodeUnknownSigner
			}
			if rev := findRevocation(q.Revocations, q.ArtifactDigest, s.KeyID); rev != nil {
				r.Code = CodeRevoked
			}
			if s.NotAfter != "" {
				if t, e := time.Parse(time.RFC3339, s.NotAfter); e == nil && !q.At.Before(t) {
					r.Code = CodeExpired
				}
			}
			if s.NotBefore != "" {
				if t, e := time.Parse(time.RFC3339, s.NotBefore); e == nil && q.At.Before(t) {
					r.Code = CodeInvalid
				}
			}
		}
