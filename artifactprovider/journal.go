...
func NewJournal(path string) (*Journal, error) {
	...
	for scanner.Scan() { var record journalRecord; if err := json.Unmarshal(scanner.Bytes(), &record); err != nil { break }; content, err := base64.StdEncoding.DecodeString(record.Content); if err != nil { return nil, err }; if record.Kind == "object" { j.objects[record.Digest] = content } else if record.Kind == "manifest" { j.manifests[record.Digest] = content } }
...
func (j *Journal) CommitManifest(ctx context.Context, content []byte) error { if err := ctx.Err(); err != nil { return err }; manifest, err := environmentartifact.DecodeManifest(content); if err != nil { return err }; j.mu.Lock(); defer j.mu.Unlock(); key := manifest.ArtifactDigest.Hex(); if prior, ok := j.manifests[key]; ok { if !bytes.Equal(prior, content) { return fmt.Errorf("conflicting immutable manifest") }; return nil }; if err := j.verifyManifestClosure(manifest); err != nil { return err }; if err := j.append(journalRecord{Kind: "manifest", Digest: key, Content: base64.StdEncoding.EncodeToString(content)}); err != nil { return err }; j.manifests[key] = append([]byte(nil), content...); return nil }
func (j *Journal) verifyManifestClosure(manifest environmentartifact.Manifest) error { refs := []environmentartifact.Descriptor{manifest.Specification.Descriptor, manifest.LegacyBlueprint.Descriptor, manifest.ObjectIndex}; refs = append(refs, manifest.Catalogs[0].Descriptor); indexBytes, ok := j.objects[manifest.ObjectIndex.Digest.Hex()]; if !ok || int64(len(indexBytes)) != manifest.ObjectIndex.Size || environmentartifact.DigestBytes(indexBytes) != manifest.ObjectIndex.Digest { return fmt.Errorf("manifest object index is not complete") }; index, err := environmentartifact.DecodeObjectIndex(indexBytes); if err != nil { return fmt.Errorf("validate manifest object index: %w", err) }; for _, entry := range index.Entries { refs = append(refs, environmentartifact.Descriptor{Digest: entry.StoredDigest, Size: entry.StoredSize}) }; for _, ref := range refs { b, ok := j.objects[ref.Digest.Hex()]; if !ok || int64(len(b)) != ref.Size || environmentartifact.DigestBytes(b) != ref.Digest { return fmt.Errorf("manifest dependency %s is not complete", ref.Digest) } }; return nil }
