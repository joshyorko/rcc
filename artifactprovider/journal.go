...
func (j *Journal) GarbageCollect(ctx context.Context,r Retention)(GCReport,error){j.adminMu.Lock();defer j.adminMu.Unlock();ms,e:=j.ListManifests(ctx);if e!=nil{return GCReport{},e};report:=GCReport{ManifestsScanned:len(ms)};keep:=map[string]bool{};now:=time.Now();kept:=0;j.mu.Lock();defer j.mu.Unlock();for _,m:=range ms{expired:=r.MaxAge>0&&!j.manifestTimes[m.Digest.Hex()].IsZero()&&now.Sub(j.manifestTimes[m.Digest.Hex()])>r.MaxAge;if expired&&(r.KeepManifests<=0||kept>=r.KeepManifests){if e=j.append(journalRecord{Kind:"delete-manifest",Digest:m.Digest.Hex()});e!=nil{return report,e};delete(j.manifests,m.Digest.Hex());delete(j.manifestTimes,m.Digest.Hex());report.ManifestsRemoved++;continue};kept++;b:=j.manifests[m.Digest.Hex()];x,e:=environmentartifact.DecodeManifest(b);if e!=nil{return report,e};keep[x.Specification.Descriptor.Digest.Hex()]=true;keep[x.LegacyBlueprint.Descriptor.Digest.Hex()]=true;keep[x.ObjectIndex.Digest.Hex()]=true;for _,c:=range x.Catalogs{keep[c.Digest.Hex()]=true}}
	for k,r:=range j.objects{if keep[k]{continue};if r.path!="" {if e=os.Remove(r.path);e==nil{if e=j.append(journalRecord{Kind:"delete-object",Digest:k});e!=nil{return report,e};delete(j.objects,k);report.ObjectsRemoved++;report.BytesReclaimed+=r.size}}};return report,nil}
