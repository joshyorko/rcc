...
type Journal struct { path, objectDir string; mu sync.RWMutex; adminMu sync.Mutex; objects map[string]objectRef; manifests map[string][]byte }
...
func (j *Journal) Capabilities(context.Context)(Capabilities,error){return Capabilities{SchemaVersions:[]int{1},DigestAlgorithms:[]string{"sha256"},Encodings:[]string{"gzip"},MaxObjectBytes:maxProviderObjectBytes,MaxManifestBytes:maxManifestBytes,MaxRequestBytes:maxProviderJSONBytes,RangeSupport:false,ResumeSupport:false},nil}
