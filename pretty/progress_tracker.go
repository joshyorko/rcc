...
func (pt *ProgressTracker) SetStep(index int, status StepStatus, message string) bool {
	pt.mu.Lock()
	defer pt.mu.Unlock()
	...
	if status == StepRunning && pt.steps[index].StartTime.IsZero() {
