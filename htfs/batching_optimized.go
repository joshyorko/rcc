...
// RestoreDirectoryOptimized is a BLAZINGLY FAST version that:
...
func RestoreDirectoryOptimized(library Library, fs *Root, current map[string]string, stats *stats) Dirtask {
		...
		return func() {
			...
			for name, subdir := range it.Dirs {
				if !subdir.Shadow && !subdir.IsSymlink() {
					...
					anywork.Backlog(RestoreDirectoryOptimized(library, fs, current, stats)(subpath, subdir))
				}
			}
			...
			for _, batch := range batches {
				if len(batch.Files) > 0 {
					anywork.Backlog(ProcessBatchOptimized(batch))
				}
			}
		}
	}
...
func processDirectoryWithLocality(path string, it *Dir, library Library, fs *Root, current map[string]string, stats *stats) []DirectoryBatch {
	...
	return createOptimizedBatches(tasksToProcess, path)
}

// createOptimizedBatches groups files for optimal processing
func createOptimizedBatches(tasks []FileTask, dirPath string) []DirectoryBatch {
	if len(tasks) == 0 {
		return nil
	}
...
// ProcessBatchOptimized processes a batch with intelligent prefetching
func ProcessBatchOptimized(batch DirectoryBatch) anywork.Work {
	return func() {
		...
		for i, task := range batch.Files {
			...
			func() {
				...
				work := DropFileOptimized(task.Library, task.Digest, task.SinkPath, task.Details, task.Rewrite)
				work()
			}()
		}
...
// DropFileOptimized is an optimized version with better buffer management
func DropFileOptimized(library Library, digest, sinkname string, details *File, rewrite []byte) anywork.Work {
	return func() {
...
// shouldBatchOptimized determines if a file should be batched
func shouldBatchOptimized(file *File) bool {
	...
	// Batch files smaller than 200KB (raised threshold for better batching)
...
func (ps *ParallelStats) Report() string {
	ps.mu.Lock()
	defer ps.mu.Unlock()

	return fmt.Sprintf("Dirs: %d, Files: %d, Bytes: %d, Dirty: %.1f%%",
		ps.dirCount, ps.fileCount, ps.byteCount, ps.Dirtyness())
}